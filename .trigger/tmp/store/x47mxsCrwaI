import {
  ...
  stepCountIs,
  tool
} from "../../../../../chunk-V3CHSFE6.mjs";
import "../../../../../chunk-R43LMS2W.mjs";
import {
  task
} from "../../../../../chunk-DXQ7XNOK.mjs";
...
async function fetchPolyfill() {
  return typeof globalThis.fetch !== "undefined" ? globalThis.fetch : (await import("../../../../../lib-DTBTQUGE.mjs")).default;
}
...
var AI_CURSOR_COLOR = "#6457f9";
var VALID_FILLS = new Set(NODE_COLORS.map((c) => c.fill));
function safeShape(s) {
  return NODE_SHAPES.includes(s) ? s : "rectangle";
}
...
var designAgent = task({
  ...
  run: /* @__PURE__ */ __name(async (payload, { ctx }) => {
    ...
    async function setAiPresence(cursor, thinking, ttl) {
      try {
    ...
    const canvasTools = {
      addNode: tool({
        description: `Add a new node to the canvas. Use descriptive kebab-case IDs like 'api-gateway' or 'user-db'. Shapes: rectangle (default), diamond (decision), circle (event), pill (service), cylinder (database), hexagon (external). Color fills: ${NODE_COLORS.map((c) => c.fill).join(", ")}.`,
        inputSchema: external_exports.object({
          id: external_exports.string().describe("Unique kebab-case node ID, e.g. 'api-gateway'"),
          label: external_exports.string().describe("Human-readable node label"),
          shape: external_exports.enum(["rectangle", "diamond", "circle", "pill", "cylinder", "hexagon"]).describe("Node shape"),
          color: external_exports.string().describe("Hex fill color from the allowed palette"),
          x: external_exports.number().describe("X canvas position in pixels"),
          y: external_exports.number().describe("Y canvas position in pixels"),
          width: external_exports.number().optional().describe("Width in pixels, default 160"),
          height: external_exports.number().optional().describe("Height in pixels, default 60")
        }),
        execute: /* @__PURE__ */ __name(async ({ id, label, shape, color, x, y, width, height }) => {
      ...
      moveNode: tool({
        ...
        inputSchema: external_exports.object({
          ...
          x: external_exports.number().describe("New X position"),
          y: external_exports.number().describe("New Y position")
        }),
        execute: /* @__PURE__ */ __name(async ({ id, x, y }) => {
          await mutateFlow({ client: liveblocks, roomId }, (flow) => {
      ...
      resizeNode: tool({
        ...
        inputSchema: external_exports.object({
          ...
          width: external_exports.number().describe("New width in pixels"),
          height: external_exports.number().describe("New height in pixels")
        }),
        execute: /* @__PURE__ */ __name(async ({ id, width, height }) => {
          await mutateFlow({ client: liveblocks, roomId }, (flow) => {
      ...
      updateNodeData: tool({
        ...
        inputSchema: external_exports.object({
          ...
          label: external_exports.string().optional().describe("New label"),
          shape: external_exports.enum(["rectangle", "diamond", "circle", "pill", "cylinder", "hexagon"]).optional().describe("New shape"),
          color: external_exports.string().optional().describe("New hex fill color from the allowed palette")
        }),
        execute: /* @__PURE__ */ __name(async ({ id, label, shape, color }) => {
          await mutateFlow({ client: liveblocks, roomId }, (flow) => {
            flow.updateNodeData(id, (data) => ({
              ...data,
              ...label !== void 0 && { label },
              ...shape !== void 0 && { shape: safeShape(shape) },
              ...color !== void 0 && { color: safeColor(color) }
            }));
          });
          return { updated: id };
        }, "execute")
      ...
      addEdge: tool({
        ...
        inputSchema: external_exports.object({
          id: external_exports.string().describe("Unique edge ID, e.g. 'gateway-to-auth'"),
        ...
        execute: /* @__PURE__ */ __name(async ({ id, source, target, label }) => {
          await mutateFlow({ client: liveblocks, roomId }, (flow) => {
            const edge = {
              id,
              type: "canvasEdge",
...
${existingNodes.length > 0 ? existingNodes.map((n) => `  id="${n.id}" label="${n.data.label}" shape=${n.data.shape} color=${n.data.color} pos=(${Math.round(n.position.x)},${Math.round(n.position.y)})`).join("\n") : "  (empty — start fresh)"}

Edges (${existingEdges.length}):
${existingEdges.length > 0 ? existingEdges.map((e) => `  id="${e.id}" ${e.source} → ${e.target}`).join("\n") : "  (none)"}
...
When the design is complete, respond with a short summary of what you built.`;
      ...
      const { text } = await generateText({
        ...
        onStepFinish: /* @__PURE__ */ __name(async ({ toolResults }) => {
          for (const r of toolResults ?? []) {
            const output = r.output;
            if (typeof output?.x === "number" && typeof output?.y === "number") {
              lastCursorPos = { x: output.x, y: output.y };
            }
          }
